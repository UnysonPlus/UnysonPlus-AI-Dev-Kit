...
async function snap(browser, url, tag) {
  ...
  const regions = await page.evaluate(() => {
    ...
    const push = (el, kind) => {
      ...
      out.push({ kind, label, id: (el.id || '').trim().toLowerCase(), y: Math.round(r.top + window.scrollY), h: Math.round(r.height) });
    };
...
function match(src, conv) {
  ...
  // (goldens assert it), so when both sides carry the same one it is the authoritative pairing and no
  // amount of text similarity should override it.
  sSecs.forEach((sc, si) => {
    if (!sc.id) return;
    const ci = cSecs.findIndex((c, k) => !usedC.has(k) && c.id && c.id === sc.id);
    if (ci < 0) return;
    usedS.add(si); usedC.add(ci); matchOf[si] = ci;
  });
  // …then BEST-PAIR-FIRST on heading text for whatever is left (not greedy per-source): score every
  // remaining source x converted pair, then assign highest-similarity pairs first. Greedy-per-source
  // mis-aligned near-duplicate headings — e.g. a source "Trusted by buyers…" (logos) would steal the
  // converted "Trusted by 2,000…" (testimonials) on the shared "trusted" token.
  const cand = [];
...
try {
  ...
  for (const pair of pairs) {
    ...
    // --only matches the section ID too, not just the heading label. Bands are PAIRED by id (the
    // converter preserves the source id), so filtering by a name that only exists as a label was
    // inconsistent: `--only reviews` silently produced zero images on a section whose heading reads
    // "What our customers say".
    const onlyHay = [key, pair.label.toLowerCase(), (pair.src && pair.src.id) || "", (pair.conv && pair.conv.id) || ""].join(" ");
    if (ONLY.length && !ONLY.some((o) => onlyHay.includes(o))) continue;
