 ...
 *   category|slug             expanded to an openhero.art preview URL
...
const expand = line =>
  /^https?:\/\//i.test(line)
    ? line
    : (line.includes('|')
        ? `https://openhero.art/api/preview?category=${line.split('|')[0].trim()}&slug=${line.split('|')[1].trim()}`
        : null);
...
/* Kept as source strings so every probe re-locates the masthead the SAME way
   ...
   OpenHero corpus the masthead is a <nav> on 72% of pages and <header> is the
