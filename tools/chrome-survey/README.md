...
node digest.mjs out/corpus-02.json                       # the report
node digest.mjs out/corpus-02.json out/corpus-01.json     # two corpora, one pass
...
`category|slug` for an <corpus base endpoint> preview, `#` comments ignored. Useful flags: `--sample 30`,
...
On the corpus-01 the masthead is a `<nav>` on **72%** of pages and `<header>` is the **hero**
...
swings the headline hard (on corpus-01: 11% → 97% of sites "needing custom CSS"), so state which mode a
