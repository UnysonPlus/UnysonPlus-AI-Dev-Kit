...
| **Survey how N SOURCE sites build their header AND footer** — corpus-scale, one-sided: masthead + footer structure, chrome, footer column ratios, both scroll states, the real hover treatment and the mobile drawer across a whole URL list, then score every construct against the theme's header settings (EXACT / NEAR / CSS) from `capability-map.json`. This is the "what do these 120 headers actually DO, and can Theme Settings express it?" pass you run BEFORE converting. Touches no localhost/DB, so it is safe alongside the converter harness. | `tools/chrome-survey/survey.mjs --urls <list> --out out/x.json` then `digest.mjs out/x.json [--format md]` | `npm i` in `tools/chrome-survey` (or reuses `tools/measure`'s playwright-core) |
| **Check a theme option is REACHABLE from a conversion** — adding a header/footer theme option does not close a converter gap; both twins (JS `to-theme-settings.mjs` + PHP `stitch`) have to EMIT it. Reports BOTH / JS-only / PHP-only / NEITHER per option id. **JS-only is a real bug** (import_dir overwrites theme-settings.json from PHP, so the rule is discarded); PHP-only is a ledger entry. Run it whenever you add a theme option for the converter, or before recording a gap as "closed" — it caught G3, closed theme-side only with neither twin emitting it. | `node tools/option-reachability/check.mjs [--all] [--json]` | none (reads source only) |
...
- **`assembled/UnysonPlus-Capture-Service/tools/design-capture/`** — the capture pipeline's own playwright
  ...
  fidelity checks.
- **`tools/chrome-survey/`** — the corpus-survey tool; resolves `playwright-core` from its own
  `node_modules`, else `tools/measure/`, else `KIT_MODS`, so it runs without a second install.
