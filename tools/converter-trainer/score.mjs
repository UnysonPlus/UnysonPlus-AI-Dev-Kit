...
// A capture lands EITHER directly at `<slug>/rendered.html` (the service's normal batch layout) or one
// level down under the dir the service named from the URL — a corpus served from a single preview
// endpoint shares that one dir name across every site in it. Probe both depths rather than hardcoding
// any corpus's dir name, so the harness works on any corpus.
function renderedPathOf(slug) {
  const flat = join(BATCH, slug, 'rendered.html');
  if (existsSync(flat)) { return flat; }
  const parent = join(BATCH, slug);
  if (!existsSync(parent)) { return ''; }
  return readdirSync(parent, { withFileTypes: true }).filter((d) => d.isDirectory())
    .map((d) => join(parent, d.name, 'rendered.html')).find(existsSync) || '';
}
function srcRenderedUrl(slug) {
  const p = renderedPathOf(slug);
...
// Baseline/scores are namespaced per CORPUS so scoring one corpus never clobbers another's baseline —
// each corpus keeps its own pair. Default '' = the unsuffixed files, for back-compat; `--corpus <id>`
...
// The fast loop runs a SAMPLE rather than the whole corpus. The sample must be STABLE across runs — a
// different subset each time makes two scores incomparable, which is the one thing a training loop cannot
// afford. It used to be a hardcoded list of source-site slugs; that named the corpus in the source tree and
// silently emptied the sample on any other corpus. Instead: take a deterministic, evenly-spaced slice of the
// sorted site list, so the same corpus always yields the same sample and any corpus yields one at all.
// A corpus that wants a curated set can drop `score/_sample.<corpus>.txt` (one slug per line, gitignored)
// and that wins.
function sampleOf(all, n) {
  const curated = join(HERE, 'score', `_sample${SUFFIX}.txt`);
  if (existsSync(curated)) {
    const picked = readFileSync(curated, 'utf8').split(/\r?\n/).map((l) => l.trim())
      .filter((l) => l && !l.startsWith('#')).filter((sl) => all.includes(sl));
    if (picked.length) { return picked.slice(0, n); }
  }
  const sorted = [...all].sort();
  if (sorted.length <= n) { return sorted; }
  const step = sorted.length / n;                       // evenly spaced = structurally varied in practice
  return Array.from({ length: n }, (_, i) => sorted[Math.floor(i * step)]);
}

function siteList() {
  if (ONLY.length) return ONLY;
  // A capturable site dir holds a rendered.html at either depth (see renderedPathOf). import-site.php
  // resolves both, so accept both here — that is what lets one harness baseline any corpus.
  const all = readdirSync(BATCH, { withFileTypes:true }).filter(d=>d.isDirectory()).map(d=>d.name)
    .filter(s => renderedPathOf(s) !== '');
  if (SAMPLE) return sampleOf(all, SAMPLE);
  return all;
}
...
async function measure(page) {
  return await page.evaluate((vw) => {
    ...
    if (header){
      ...
      // (corpus-02 a spa fixture and ~others). Prefer a logo/brand-classed element; else the anchor/element that
    ...
    // silently going missing scored 100 on every other dimension (corpus-02 audit §8.54); this makes it visible.
...
/**
 * Run one site's import with a deadline that ACTUALLY ENDS IT.
 *
 * execFileSync's `timeout` does not reliably fire on Windows, and it signals only the process it
 * spawned — so an import that outruns its deadline (measured: ~14s standalone, ~37s spawned from node,
 * and it does not return at all once a Playwright Chrome is up alongside it) STALLED the whole corpus
 * run. One slow site then cost every later site, and the run printed nothing at all, which reads as a
 * broken harness rather than a slow one.
 *
 * Async spawn + an explicit timer, and on Windows a `taskkill /T /F` that ends the process TREE (PHP can
 * outlive a signal to its own pid). Rejecting here is exactly what the caller already handles: that site
 * reports IMPORT-FAIL and the loop carries on.
 *
 * STDERR IS DISCARDED deliberately: the import emits PHP warnings and deprecations there, and a full
 * stderr pipe blocks the child while the parent drains only stdout. The JSON verdict comes on stdout.
 *
 * @param {string} php   the PHP binary
 * @param {string[]} args  script + slug + batch root
 * @param {number} ms    deadline in milliseconds
 * @returns {Promise<string>} stdout
 */
/**
 * Give an awaited step a deadline it cannot outlive.
 *
 * Every per-site step below runs against a page the converter produced, which on a bad conversion can be
 * pathological — a runaway body height, a font that never settles, an evaluate that never returns. None
 * of these had a cap, so one such page stalled the ENTIRE corpus run: no site line was ever printed and
 * every later site went unmeasured, which reads as a broken harness rather than one bad page.
 * Playwright's own `timeout` covers goto and waitForFunction, but not `evaluate`.
 *
 * @param {Promise} p      the step
...
function withDeadline(p, ms, label) {
  ...
  return Promise.race([
    p,
    new Promise((_, rej) => { t = setTimeout(() => rej(Object.assign(new Error(label + ' timed out'), { code: 'ESTEP', step: label })), ms); }),
  ]).finally(() => clearTimeout(t));
}

function runImport(php, args, ms) {
  return new Promise((resolve, reject) => {
    const child = spawn(php, args, { stdio: ['ignore', 'pipe', 'ignore'], windowsHide: true });
    let out = '';
    let done = false;
    const finish = (err, val) => { if (done) return; done = true; clearTimeout(timer); err ? reject(err) : resolve(val); };
    const timer = setTimeout(() => {
      const e = new Error('import timed out'); e.code = 'ETIMEDOUT';
      try {
    ...
    child.on('close', (code) => (code === 0 ? finish(null, out) : finish(Object.assign(new Error('import exited ' + code), { code: 'EEXIT' }))));
  });
}
...
function scoreSite(m, php, drift = null) {
  ...
  // image band vanishing (a personal-blog fixture's 3-card article grid, corpus-02 audit §8.54): seven dimensions read
...
(async () => {
  ...
  for (const slug of sites) {
    ...
    // throw → a 100% FALSE IMPORT-FAIL for every site. Strip NULs first. (corpus-02 render audit §8.55.)
    // The import runs through runImport(): async spawn with a deadline that actually ends the process
    // TREE. See its docblock for why execFileSync could not do this. A site that fails or outruns the
    // deadline is reported and skipped; the run continues.
    try { php = JSON.parse((await runImport(PHP, [join(HERE,'score','import-site.php'), slug, BATCH], 90000)).replace(/\0/g,'').trim().split('\n').pop()); }
    catch(e){ console.log(`  ${slug.padEnd(38)} IMPORT-FAIL${e && e.signal ? ` (${e.signal})` : (e && e.code === 'ETIMEDOUT' ? ' (timeout)' : '')}`); continue; }
    ...
    try {
      await withDeadline(page.evaluate(async () => {
        // CAP THE PASS. The step count follows the page height, and a runaway conversion can report a
        // body tens of thousands of pixels tall — thousands of 45ms steps, i.e. minutes inside one
        // evaluate that nothing interrupts. Forty steps reaches ~24000px, past any real page.
        const h = Math.min(document.body.scrollHeight, 600 * 40);
        for (let y = 0; y < h; y += 600) { window.scrollTo(0, y); await new Promise(r => setTimeout(r, 45)); }
        window.scrollTo(0, 0);
      }), 45000, 'lazy-image pass');
      await page.waitForTimeout(500);
    } catch (e) {}
    let m;
    try { m = await withDeadline(measure(page), 60000, 'measure'); }
    catch (e) { console.log(`  ${slug.padEnd(38)} MEASURE-FAIL${e && e.code === 'ESTEP' ? ' (timeout)' : ''}`); continue; }
