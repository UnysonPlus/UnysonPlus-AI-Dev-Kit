...
demo sites** (openhero, wegic, or any URL list). Instead of screenshotting every demo,
...
```
sites/openhero.txt   category|slug (or full URLs), one per line   ← the corpus
...
baseline (`_baseline.<name>.json`) and can't clobber another's — e.g. `--corpus wegic` alongside the default
openhero. It enumerates both capture layouts: openhero's nested `<slug>/openhero_art_api_preview/rendered.html`
...
4. **Re-audit only** (fast, no browser): `bash train.sh sites/openhero.txt --audit-only` —
...
node capture.mjs sites/openhero.txt --limit 8
...
bash train.sh sites/openhero.txt --audit-only

# a different corpus / arbitrary URLs
printf 'https://my-website-h3eknqdj.wegic.net/\n' > sites/wegic.txt
bash train.sh sites/wegic.txt
```

## Adding to the corpus

`sites/openhero.txt` is `category|slug` per line (the openhero `/api/preview?category=..&slug=..`
