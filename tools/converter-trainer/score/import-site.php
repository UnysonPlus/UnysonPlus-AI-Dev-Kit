...
// The capture service names its output dir from the URL, so a corpus served from ONE preview endpoint
// shares a single dir name across every site in it — hence the per-slug parent. RESOLVE that dir rather
// than hardcoding any corpus's name: the bundle sits either directly under <slug>/ or one level below it.
$dir = "$batch/$slug";
if ( ! file_exists( "$dir/rendered.html" ) ) {
	foreach ( (array) glob( "$batch/$slug/*", GLOB_ONLYDIR ) as $sub ) {
		if ( file_exists( "$sub/rendered.html" ) ) { $dir = $sub; break; }
	}
}
...
// photos was absent — corpus-02 audit §8.54). Count raster <img> in the body, EXCLUDING: site chrome regions
