...
- Carousel sub-options (revealed by carousel/slider designs — `default`, `split`, `thumbnav`, `spotlight`, `pullquote`): `carousel_autoplay` (`yes`/`no`), `carousel_interval` (ms string), `carousel_pause_hover` (`yes`/`no`), `carousel_controls` (`yes`/`no`), `carousel_indicators` (`yes`/`no`), `carousel_indicator_style` (`none` `dots` `lines`), `carousel_wrap` (`yes`/`no`). Other design-specific options: `default.items_per_slide` (`1` `2` `3`), `default.card_style` (`''` Plain, `card card-body` Card, `card card-body border` Bordered, `card card-body shadow` Shadow, `card card-body bg-light` Light, `card card-body bg-dark text-light` Dark), `default.avatar_position` (`top` `left` `right` `none`); `marquee.marquee_speed` (`slow` `normal` `fast`) + `marquee.marquee_direction` (`left` = Right→Left, `right` = Left→Right); `masonry.masonry_columns` / `bubble.bubble_columns` (`1` `2` `3` `4`); `zigzag.zigzag_start` (`left` `right`).
