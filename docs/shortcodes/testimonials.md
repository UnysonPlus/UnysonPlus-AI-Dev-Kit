...
|---|---|---|---|---|
| `testimonials` | addable-popup | `[]` | array of `{ content, author_avatar, author_name, author_job, site_name, site_url, rating, extra }` | The testimonial entries. |
...
| `card_rows` | addable-popup | seeded | array of `{ slots:[…], direction, justify, align, reverse }` | **Card Rows** slot designer (own **Card tab**, cross-design; the Classic design's cards render from it). The composable card model shared with `wc_products` / `team-member` / `posts` via `sc_card_rows_field()`. Slots: `quotemark, quote, avatar, name, role, **author** (=name+role stacked), **identity** (=avatar+name+role), rating, site, **extra** (=the Extra Texts stat/result footer rows)`. Avatar position = which row + its direction (inline = beside, stacked = above/below); a row's **`reverse`** flips an inline row (avatar left↔right) without re-ordering slots. *(Replaced `card_style`, `avatar_position`, `show_rating`, removed 2026-08-01: presence = "is the slot in a row".)* |
