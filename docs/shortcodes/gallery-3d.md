...
## Notes
- `design_settings` is a **multi-picker**: `{ "design": "<slug>", "<slug>": { …that design's controls… } }`. Only the active branch is required; the safe generator emits all five defaulted plus the active one. Designs: `carousel_ring` · `panorama_wall` · `card_sphere` · `orbit_globe` · `photo_scatter` · `device_cycler`.
